...
int align (alignentry*** R, alignentry*** D, alignentry*** I, char* s1, int len1, char* s2, int len2, int open, int extend)
{
  ...
  for (i = 1; i <= len1; i++)
  {
    for (j = 1; j <= len2; j++)
    {
      ...
      if (cost > INF)
        cost = INF;
      R[i][j] = alignentry_new(cost, rv, dv, iv);
      ...
      D[i][j] = alignentry_new(cost, rv, dv, iv);
      ...
      if (cost > INF)
        cost = INF;
      I[i][j] = alignentry_new(cost, rv, dv, iv);
    }
  }
  // return costs for last element in table
  //printf("%d, %d, %d\n", R[len1][len2]->value, D[len1][len2]->value, I[len1][len2]->value);

...
static int msa_validate (Msa* msa)
{
  ...
  len = msa->entries->entries[0]->length;
  /* check if all s' have the same length */
  for (i = 1; i < msa->length; i++)
  {
    if (msa->entries->entries[i]->length != len)
      return 1;
  }
  ...
  */
  for (i = 0; i < len; i++)
  {
    ...
    {
      if(msa->entries->entries[j]->sequence[i] != '-')
        nogap = 1;
    }
    if (!nogap)
      return 1;
...
Msa* msa_initialize (char* filename)
{
  ...
  msa->entries = read_fasta_file(filename);
  if (msa->entries == NULL)
  {
    free(msa);
    return NULL;
...
static void msa_show(Msa* msa)
{
  ...
  {
    printf(">%s\n",msa->entries->entries[i]->header);
    printf("%s\n",msa->entries->entries[i]->sequence);
  }
}
...
int msa_sum_of_pairs(Msa* msa, int(*score)(void*, char, char), void* scorematrix)
{
  ...
  for (i = 0; i < msa->entries->entries[0]->length; i++)
  {
    for (j = 0; j < msa->length; j++)
    {
      for (k = j+1; k < msa->length; k++)
      {
        sc += score(scorematrix, msa->entries->entries[j]->sequence[i],
            msa->entries->entries[k]->sequence[i]);
      }
    }
  }
...
int msa_consensus_score(Msa* msa, int (*score)(void*, char, char), void* scorematrix)
{
  ...
  for (i = 0; i < msa->entries->entries[0]->length; i++)
  {
    ...
    */
    for (j = 0; j < msa->length; j++)
    {
      pos = msa->entries->entries[j]->sequence[i];
    ...
    for (j = 0; j < msa->length; j++)
    {
      sc += score(scorematrix,cons,msa->entries->entries[j]->sequence[i]);
    }
  }
